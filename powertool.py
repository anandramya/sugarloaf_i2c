...
class PowerToolI2C (object) :
    ...
    def Read_IOUT_OC_FAULT_LIMIT(self, page):
        """Read IOUT_OC_FAULT_LIMIT register (0x46) - Linear11 format"""
        data = self.i2c_read16PMBus(page, PMBusDict["IOUT_OC_FAULT_LIMIT"])
    ...
    def Write_IOUT_OC_FAULT_LIMIT(self, page, current_amps):
        """Write IOUT_OC_FAULT_LIMIT register (0x46) - Linear11 format"""
        # Convert current in Amperes to Linear11 format
        # Find appropriate exponent (typically -5 to 10 for current)
        exponent = 0
        mantissa = current_amps

        # Adjust exponent and mantissa to fit in Linear11 range
        # Mantissa must fit in 11 bits signed (-1024 to 1023)
        while mantissa >= 1024 and exponent < 15:
            mantissa = mantissa / 2.0
            exponent += 1

        while mantissa < 512 and mantissa != 0 and exponent > -16:
            mantissa = mantissa * 2.0
            exponent -= 1
        ...
        # Handle negative exponent for two's complement (5-bit)
        if exponent < 0:
            exponent_bits = (1 << 5) + exponent  # Two's complement
        else:
            exponent_bits = exponent

        # Combine into Linear11 format
        raw_value = ((exponent_bits & 0x1F) << 11) | (mantissa & 0x7FF)

        # Write to register
        self.i2c_write16PMBus(page, PMBusDict["IOUT_OC_FAULT_LIMIT"], raw_value)

        return True

    def Read_OT_FAULT_LIMIT(self, page):
        """Read OT_FAULT_LIMIT register (0x4F) - Linear11 format"""
        data = self.i2c_read16PMBus(page, PMBusDict["OT_FAULT_LIMIT"])
    ...
    def Write_OT_FAULT_LIMIT(self, page, temp_celsius):
        """Write OT_FAULT_LIMIT register (0x4F) - Linear11 format"""
        # Convert temperature in Celsius to Linear11 format
        # Find appropriate exponent (typically -5 to 10 for temperature)
        exponent = 0
        mantissa = temp_celsius
        ...
        # Write to register
        self.i2c_write16PMBus(page, PMBusDict["OT_FAULT_LIMIT"], raw_value)

        return True

    def Read_VOUT_UV_FAULT_LIMIT(self, page):
        """Read VOUT_UV_FAULT_LIMIT register (0x44) - Linear16 format"""
        data = self.i2c_read16PMBus(page, PMBusDict["VOUT_UV_FAULT_LIMIT"])
        ...
        # Linear16 format: mantissa * 2^exponent
        vout_uv_limit = data * math.pow(2, exponent)

        return vout_uv_limit

    def Write_VOUT_UV_FAULT_LIMIT(self, page, voltage):
        """Write VOUT_UV_FAULT_LIMIT register (0x44) - Linear16 format"""
        # Read VOUT_MODE to get the correct exponent
        try:
            exponent = self.Read_VOUT_MODE(page)
        except:
        ...
        # Convert voltage to raw value using Linear16 format
        # raw_value = voltage / (2^exponent)
        raw_value = int(round(voltage / math.pow(2, exponent)))

        # Ensure raw_value fits in 16 bits (0-65535)
        if raw_value < 0 or raw_value > 65535:
            print(f"Error: Raw value {raw_value} out of range for 16-bit register")
            return False

        # Write to register
        self.i2c_write16PMBus(page, PMBusDict["VOUT_UV_FAULT_LIMIT"], raw_value)
...
def test_iout_oc_fault_limit(rail, new_current=None):
    """Test IOUT_OC_FAULT_LIMIT read, write, and verify on specified rail

    Args:
        rail: Rail name (TSP_CORE or TSP_C2C)
        new_current: Optional new current limit in Amperes. If None, reads current value only.
    """
    ...
    try:
        ...
        if new_current is not None:
            print(f"IOUT_OC_FAULT_LIMIT (0x46) - Write and Verify on {rail} (Page {page})")
        else:
            print(f"IOUT_OC_FAULT_LIMIT (0x46) - Read on {rail} (Page {page})")
        print(f"{'='*70}")
        ...
        print(f"\nStep 1: Reading current IOUT_OC_FAULT_LIMIT value...")
        current_limit = powertool.Read_IOUT_OC_FAULT_LIMIT(page)
        raw_value = powertool.i2c_read16PMBus(page, PMBusDict["IOUT_OC_FAULT_LIMIT"])
        print(f"  Current limit: {current_limit:.3f} A")
        print(f"  Raw hex value: 0x{raw_value:04X}")

        # If no new value specified, just read and return
        if new_current is None:
            print(f"\n✓ Read completed successfully")
            return True

        # Validate new current value
        if new_current < 0 or new_current > 2000:
            print(f"Error: Current {new_current}A is outside reasonable range (0A to 2000A)")
            return False

        # Step 2: Set a new value
        print(f"\nStep 2: Writing new IOUT_OC_FAULT_LIMIT value...")
        test_value = new_current
        print(f"  Setting limit to: {test_value:.3f} A")
        powertool.Write_IOUT_OC_FAULT_LIMIT(page, test_value)

        # Step 3: Wait briefly for write to complete
        time.sleep(0.1)

        # Step 4: Verify the written value
        print(f"\nStep 3: Verifying written value...")
        verify_limit = powertool.Read_IOUT_OC_FAULT_LIMIT(page)
        verify_raw = powertool.i2c_read16PMBus(page, PMBusDict["IOUT_OC_FAULT_LIMIT"])
        print(f"  Verified limit: {verify_limit:.3f} A")
        print(f"  Verified hex value: 0x{verify_raw:04X}")

        # Check if values match within tolerance
        tolerance = test_value * 0.02  # 2% tolerance for Linear11 conversion
        if abs(verify_limit - test_value) <= tolerance:
            print(f"\n✓ SUCCESS: Write and verify successful (within ±2% tolerance)")
            print(f"  Original: {current_limit:.3f} A (0x{raw_value:04X})")
            print(f"  Written:  {test_value:.3f} A")
            print(f"  Verified: {verify_limit:.3f} A (0x{verify_raw:04X})")
            return True
        else:
            print(f"\n⚠ WARNING: Verified value differs from written value")
            print(f"  Difference: {abs(verify_limit - test_value):.3f} A")
            return False

    except Exception as e:
        print(f"\nError testing IOUT_OC_FAULT_LIMIT on {rail}: {e}")
        import traceback
        traceback.print_exc()
        return False

def test_ot_fault_limit(rail, new_temp=None):
    """Test OT_FAULT_LIMIT read, write, and verify on specified rail

    Args:
        rail: Rail name (TSP_CORE or TSP_C2C)
        new_temp: Optional new temperature limit in Celsius. If None, reads current value only.
    """
    ...
    try:
        ...
        if new_temp is not None:
            print(f"OT_FAULT_LIMIT (0x4F) - Write and Verify on {rail} (Page {page})")
        else:
            print(f"OT_FAULT_LIMIT (0x4F) - Read on {rail} (Page {page})")
        print(f"{'='*70}")
        ...
        # Step 1: Read current value
        print(f"\nStep 1: Reading current OT_FAULT_LIMIT value...")
        current_limit = powertool.Read_OT_FAULT_LIMIT(page)
        raw_value = powertool.i2c_read16PMBus(page, PMBusDict["OT_FAULT_LIMIT"])
        print(f"  Current limit: {current_limit:.2f} °C")
        print(f"  Raw hex value: 0x{raw_value:04X}")
        ...
        # Validate new temperature value
        if new_temp < 0 or new_temp > 200:
            print(f"Error: Temperature {new_temp}°C is outside reasonable range (0°C to 200°C)")
            return False

        # Step 2: Set a new value
        print(f"\nStep 2: Writing new OT_FAULT_LIMIT value...")
        test_value = new_temp
        print(f"  Setting limit to: {test_value:.2f} °C")
        powertool.Write_OT_FAULT_LIMIT(page, test_value)
        ...
        # Step 4: Verify the written value
        print(f"\nStep 3: Verifying written value...")
        verify_limit = powertool.Read_OT_FAULT_LIMIT(page)
        verify_raw = powertool.i2c_read16PMBus(page, PMBusDict["OT_FAULT_LIMIT"])
        print(f"  Verified limit: {verify_limit:.2f} °C")
        print(f"  Verified hex value: 0x{verify_raw:04X}")

        # Check if values match within tolerance
        tolerance = max(test_value * 0.02, 1.0)  # 2% tolerance or 1°C minimum
        if abs(verify_limit - test_value) <= tolerance:
            print(f"\n✓ SUCCESS: Write and verify successful (within ±2% tolerance)")
            print(f"  Original: {current_limit:.2f} °C (0x{raw_value:04X})")
            print(f"  Written:  {test_value:.2f} °C")
            print(f"  Verified: {verify_limit:.2f} °C (0x{verify_raw:04X})")
            return True
        else:
            print(f"\n⚠ WARNING: Verified value differs from written value")
            print(f"  Difference: {abs(verify_limit - test_value):.2f} °C")
            return False

    except Exception as e:
        print(f"\nError testing OT_FAULT_LIMIT on {rail}: {e}")
        import traceback
        traceback.print_exc()
        return False

def test_vout_uv_fault_limit(rail, new_voltage=None):
    """Test VOUT_UV_FAULT_LIMIT read, write, and verify on specified rail

    Args:
        rail: Rail name (TSP_CORE or TSP_C2C)
        new_voltage: Optional new voltage limit in Volts. If None, reads current value only.
    """
    ...
    try:
        ...
        if new_voltage is not None:
            print(f"VOUT_UV_FAULT_LIMIT (0x44) - Write and Verify on {rail} (Page {page})")
        else:
            print(f"VOUT_UV_FAULT_LIMIT (0x44) - Read on {rail} (Page {page})")
        print(f"{'='*70}")
        ...
        # Step 1: Read current value
        print(f"\nStep 1: Reading current VOUT_UV_FAULT_LIMIT value...")
        current_limit = powertool.Read_VOUT_UV_FAULT_LIMIT(page)
        raw_value = powertool.i2c_read16PMBus(page, PMBusDict["VOUT_UV_FAULT_LIMIT"])
        print(f"  Current limit: {current_limit:.6f} V")
        print(f"  Raw hex value: 0x{raw_value:04X}")
        ...
        # Validate new voltage value
        if new_voltage < 0.1 or new_voltage > 3.3:
            print(f"Error: Voltage {new_voltage}V is outside reasonable range (0.1V to 3.3V)")
            return False

        # Step 2: Set a new value
        print(f"\nStep 2: Writing new VOUT_UV_FAULT_LIMIT value...")
        test_value = new_voltage
        print(f"  Setting limit to: {test_value:.6f} V")
        powertool.Write_VOUT_UV_FAULT_LIMIT(page, test_value)
        ...
        # Step 4: Verify the written value
        print(f"\nStep 3: Verifying written value...")
        verify_limit = powertool.Read_VOUT_UV_FAULT_LIMIT(page)
        verify_raw = powertool.i2c_read16PMBus(page, PMBusDict["VOUT_UV_FAULT_LIMIT"])
        print(f"  Verified limit: {verify_limit:.6f} V")
        print(f"  Verified hex value: 0x{verify_raw:04X}")

        # Check if values match within tolerance
        tolerance = test_value * 0.01  # 1% tolerance for voltage
        if abs(verify_limit - test_value) <= tolerance:
            print(f"\n✓ SUCCESS: Write and verify successful (within ±1% tolerance)")
            print(f"  Original: {current_limit:.6f} V (0x{raw_value:04X})")
            print(f"  Written:  {test_value:.6f} V")
            print(f"  Verified: {verify_limit:.6f} V (0x{verify_raw:04X})")
            return True
        else:
            print(f"\n⚠ WARNING: Verified value differs from written value")
            print(f"  Difference: {abs(verify_limit - test_value):.6f} V")
            return False

    except Exception as e:
        print(f"\nError testing VOUT_UV_FAULT_LIMIT on {rail}: {e}")
        import traceback
        traceback.print_exc()
        return False
...
Note: Add "log" as third argument to continuously log any command to CSV file
    ...
    if args.rail and args.command:
        ...
        # Handle IOUT_OC_FAULT_LIMIT test command
        if command == "IOUT_OC_FAULT_LIMIT" or command == "TEST_IOUT_OC":
            # Check if a current value is provided
            if args.log_mode:
                try:
                    current_value = float(args.log_mode)
                    success = test_iout_oc_fault_limit(rail, current_value)
                    sys.exit(0 if success else 1)
                except ValueError:
                    print("Error: For IOUT_OC_FAULT_LIMIT, the third argument must be a current value in Amperes (e.g., 500 for 500A)")
                    sys.exit(1)
            else:
                # Just read the current value
                success = test_iout_oc_fault_limit(rail)
                sys.exit(0 if success else 1)

        # Handle OT_FAULT_LIMIT test command
        elif command == "OT_FAULT_LIMIT" or command == "TEST_OT":
            # Check if a temperature value is provided
            if args.log_mode:
                try:
                    temp_value = float(args.log_mode)
                    success = test_ot_fault_limit(rail, temp_value)
                    sys.exit(0 if success else 1)
                except ValueError:
                    print("Error: For OT_FAULT_LIMIT, the third argument must be a temperature value in Celsius (e.g., 125 for 125°C)")
                    sys.exit(1)
            else:
                # Just read the current value
                success = test_ot_fault_limit(rail)
                sys.exit(0 if success else 1)

        # Handle VOUT_UV_FAULT_LIMIT test command
        elif command == "VOUT_UV_FAULT_LIMIT" or command == "TEST_VOUT_UV":
            # Check if a voltage value is provided
            if args.log_mode:
                try:
                    voltage_value = float(args.log_mode)
                    success = test_vout_uv_fault_limit(rail, voltage_value)
                    sys.exit(0 if success else 1)
                except ValueError:
                    print("Error: For VOUT_UV_FAULT_LIMIT, the third argument must be a voltage value in Volts (e.g., 0.7 for 0.7V)")
                    sys.exit(1)
            else:
                # Just read the current value
                success = test_vout_uv_fault_limit(rail)
                sys.exit(0 if success else 1)

        # Handle VOUT_COMMAND (voltage setting)
        elif command == "VOUT_COMMAND":
            if args.log_mode:
                try:
